...
static const rclcpp::Logger LOGGER = rclcpp::get_logger("lbr_servo.servo_command.cpp");
...
void publishCommands()
{
  ...
  if (count_ < 10)
  {
    ...
    msg->joint_names.push_back("lbr_joint_6");
...
int main(int argc, char** argv)
{
  ...
  node_ = std::make_shared<rclcpp::Node>("servo_command");
  ...
  if (planning_scene_monitor->getPlanningScene())
  {
    ...
    planning_scene_monitor->setPlanningScenePublishingFrequency(
            100);
  ...
  joint_cmd_pub_ = node_->create_publisher<control_msgs::msg::JointJog>("/servo_command/delta_joint_cmds", 10);
  twist_cmd_pub_ = node_->create_publisher<geometry_msgs::msg::TwistStamped>("/servo_command/delta_twist_cmds", 10);
  ...
  collision_object.header.frame_id = "lbr_link_0";
